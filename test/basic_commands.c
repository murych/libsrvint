...
static int server_loop(int fd) {
  for (int command_number = 0; command_number < 8; ++command_number) {
    ...
    if (request_length != 0 &&
        read_all(fd, request + 6, request_length + 1) != 0)
      return 3;
...
int main(void) {
  ...
  result |= srvint_unknown(ctx, 0x99, unknown_operands,
                           sizeof(unknown_operands), &value) ||
            value != 0x77;
  ...
  result |=
      set_length != 4 || set_response[0] != 0xb0 || set_response[3] != 0x88;
  ...
  result |=
      get_length != 3 || get_response[0] != 0xb2 || get_response[2] != 0x99;
